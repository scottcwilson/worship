<?php
    ...
    * 
    *  05/2025  project: square_webPay v3.0.0 file: square_admin_notification.php
    */
...
if (!empty($transaction) && $transaction->getId()) {
    ...
    $outputSquare .= MODULE_PAYMENT_SQ_WEBPAY_ENTRY_TRANSACTION_SUMMARY . ':' . "\n";
    ...
    foreach ($tenders as $tender) {
        ...
        $amount = $GLOBALS['currencies']->format($this->convertFromBaseCurrencyUnit($tender->getAmountMoney()->getAmount(), $currency_code), false, $currency_code);
        $outputSquare .= '<tr><td class="main">' . "\n";
    ...
    if ($refunds && count($refunds)) {
        ...
        foreach ($refunds as $refund) {
            ...
            $amount = $GLOBALS['currencies']->format($this->convertFromBaseCurrencyUnit($refund->getAmountMoney()->getAmount(), $currency_code), false, $currency_code);
            $outputSquare .= '<tr><td class="main">' . "\n";
...
if (method_exists($this, '_doRefund')) {
    ...
    $outputRefund .= '<td class="main">' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_REFUND_TITLE . '<br>' . "\n";
    ...
    $outputRefund .= MODULE_PAYMENT_SQ_WEBPAY_ENTRY_REFUND . '<br>';
    $outputRefund .= MODULE_PAYMENT_SQ_WEBPAY_ENTRY_REFUND_AMOUNT_TEXT . ' ' . zen_draw_input_field('refamt', '', 'length="10" placeholder="amount"') . '<br>';
    $outputRefund .= MODULE_PAYMENT_SQ_WEBPAY_TEXT_REFUND_CONFIRM_CHECK . zen_draw_checkbox_field('refconfirm', '', false) . '<br>';
    $outputRefund .= '<br>' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_REFUND_TEXT_COMMENTS . '<br>' . zen_draw_textarea_field('refnote', 'soft', '50', '3', MODULE_PAYMENT_SQ_WEBPAY_ENTRY_REFUND_DEFAULT_MESSAGE);
    $outputRefund .= '<br>' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_REFUND_SUFFIX;
    $outputRefund .= '<br><input type="submit" name="buttonrefund" value="' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_REFUND_BUTTON_TEXT . '" title="' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_REFUND_BUTTON_TEXT . '">';
    $outputRefund .= '</form>';
...
if (method_exists($this, '_doCapt')) {
    ...
    $outputCapt .= '<td class="main">' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_CAPTURE_TITLE . '<br>' . "\n";
    ...
    $outputCapt .= MODULE_PAYMENT_SQ_WEBPAY_ENTRY_CAPTURE . '<br>';
    $outputCapt .= MODULE_PAYMENT_SQ_WEBPAY_TEXT_CAPTURE_CONFIRM_CHECK . zen_draw_checkbox_field('captconfirm', '', false) . '<br>';
    $outputCapt .= '<br>' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_CAPTURE_TEXT_COMMENTS . '<br>' . zen_draw_textarea_field('captnote', 'soft', '50', '2', MODULE_PAYMENT_SQ_WEBPAY_ENTRY_CAPTURE_DEFAULT_MESSAGE);
    $outputCapt .= '<br>' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_CAPTURE_SUFFIX;
    $outputCapt .= '<br><input type="submit" name="btndocapture" value="' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_CAPTURE_BUTTON_TEXT . '" title="' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_CAPTURE_BUTTON_TEXT . '">';
    $outputCapt .= '</form>';
...
if (method_exists($this, '_doVoid')) {
    ...
    $outputVoid .= '<td class="main">' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_VOID_TITLE . '<br>' . "\n";
    ...
    $outputVoid .= MODULE_PAYMENT_SQ_WEBPAY_ENTRY_VOID;
    $outputVoid .= '<br>' . MODULE_PAYMENT_SQ_WEBPAY_TEXT_VOID_CONFIRM_CHECK . zen_draw_checkbox_field('voidconfirm', '', false);
    $outputVoid .= '<br><br>' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_VOID_TEXT_COMMENTS . '<br>' . zen_draw_textarea_field('voidnote', 'soft', '50', '3', MODULE_PAYMENT_SQ_WEBPAY_ENTRY_VOID_DEFAULT_MESSAGE);
    $outputVoid .= '<br>' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_VOID_SUFFIX;
    $outputVoid .= '<br><input type="submit" name="ordervoid" value="' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_VOID_BUTTON_TEXT . '" title="' . MODULE_PAYMENT_SQ_WEBPAY_ENTRY_VOID_BUTTON_TEXT . '">';
    $outputVoid .= '</form>';
